...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    $log_msg = "[" . date('Y-m-d H:i:s') . "] Auth Attempt: User=[" . $user . "] | DB_User=" . ($admin['admin_user'] ?? 'N/A') . " | Found=" . ($admin ? 'YES' : 'NO') . " | Pass_Verify=" . ($admin && password_verify($pass, $admin['admin_pass']) ? 'YES' : 'NO') . "\n";
    @file_put_contents(__DIR__ . '/logs/auth_diag.log', $log_msg, FILE_APPEND);
